...
impl<'a> Chunk<'a> for Sprites {
    ...
    fn read<R: GameDataRead>(reader: &mut R) -> Result<Self::ReadOutput, ReadError> {
        ...
        let start_offset = reader.tell()? as usize;
        trace!("{} sprites", count);
        // Read offset table
        ...
        while let Some(reader_offset) = sprite_offsets.next() {
            let reader_offset = reader.seek(io::SeekFrom::Start(reader_offset as u64))? as usize;
    ...
    fn write_content<W: GameDataWrite>(&self, writer: &mut W) -> io::Result<()> {
        ...
        let mut offset = writer.tell()? as u32 + (count * 4);
        // Write offset table
        ...
        for s in &self.sprites {
            ...
            writer.write_u32::<LittleEndian>(s.width)?;
            writer.write_u32::<LittleEndian>(s.height)?;
            writer.write_all(&s.unknown)?;
        }
        Ok(())
    }
}
